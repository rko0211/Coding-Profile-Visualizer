...
function Veridict({ data1, uname }) {
  ...
  for (let i = 0; i < data1[2].result.length; i++) {
    ...
    if (myMap.has(data1[2].result[i].verdict)) {
      myMap.set(data1[2].result[i].verdict, myMap.get(data1[2].result[i].verdict) + 1);
    }
    else {
      myMap.set(data1[2].result[i].verdict, 1);
    }
  ...
  return (
    <div style={{ maxWidth: '800px' }} className='my-16 mx-auto shadow-xl rounded-lg'>
      <h2 style={{ textAlign: 'center' }} className='text-2xl font-bold text-center mb-4'>Veridicts of {uname}</h2>
